// src/app/api/tx/welcome/route.ts
...
function welcomeHtml(name?: string | null) {
  ...
  // Simple, bulletproof table-based layout for email clients
  return `
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!snap.exists) {
      // If somehow the customer doc isn't initialized yet, create a minimal one.
      await ref.set(
        {
          uid,
    ...
    // Re-read (or use prior data)
    const data = (await ref.get()).data() || {};
    const alreadySent = data?.tx?.welcomeSentAt;

    if (alreadySent) {
      // Idempotent: don't send again
