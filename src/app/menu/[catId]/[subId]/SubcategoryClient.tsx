// src/app/menu/[catId]/[subId]/SubcategoryClient.tsx
"use client";
...
export default function SubcategoryClient({ catId, subId }: { catId: string; subId: string }) {
  ...
  // Carga categoría/subcat y items
  useEffect(() => {
    ...
    (async () => {
      ...
      const qItems = query(collection(db, "menuItems"), where("subcategoryId", "==", realSubId));
  ...
  async function onToggleOptions(mi: MenuItem) {
    ...
    if (!nowExpanded) return; // cerrando
    ...
    if (ogIds.length) {
      ...
      if (missing.length) {
        ...
        for (const gid of missing) {
          const snap = await getDoc(doc(db, "option-groups", gid));
          if (snap.exists()) {
      ...
      for (const gid of ensureIds) {
        if (!groupItems[gid]) {
          const q = query(collection(db, "option-items"), where("groupId", "==", gid));
