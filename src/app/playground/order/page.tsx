...
type MenuItem = {
  ...
  price?: number | null;       // precio normal (opcional si usas priceCents)
  priceCents?: number | null;  // preferido si existe
...
export default function OrderPlaygroundPage() {
  ...
  /* ✅ Enviar orden: payload con `lines` exactamente */
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 12 }}>
            {items
              ...
              .map((mi) => {
                const p = typeof mi.priceCents === "number"
                  ? mi.priceCents / 100
                  : typeof mi.price === "number"
                  ? mi.price
                  : 0;
          ...
          {/* Error/OK */}
          {errorMsg && (
            <div style={{ marginTop: 12, color: "#b91c1c" }}>{errorMsg}</div>
          )}
          {okMsg && (
            <div style={{ marginTop: 12, color: "#15803d" }}>{okMsg}</div>
          )}
